...
if nargin < 3 || isempty(fault_type)
    fault_type = 'normal';
end
...
SOC_end      = X_d(end, 1);   % SOC after discharge (depleted state)
...
if length(t_c) >= 2
    ...
    V_c         = V_d;
    x_next      = X_d(end, :)';
    warning('simulate_cycle: cycle %d — charge phase returned < 2 points.', cycle_num);
end

% =========================================================================
% CAPACITY FEATURES
% =========================================================================
SOH_now = x_next(3);              % SOH at end of cycle (after both phases)
...
row.Tmin_C             = T_min_C;
row.Tmax_C             = T_max_C;
row.chargetime_min     = actual_ch_s / 60;   % actual solver time (min)
row.discharge_time_min = actual_dch_s / 60;  % actual solver time (min)
row.x_next             = x_next;
row.SOH_end            = x_next(3);
row.fault_type         = fault_type;         % FIX 2: now properly defined
