// app_uart_protocol.c
// Uses huart1 (USART1, PA9/PA10) - the DATA link to MATLAB.
// huart2 is this board's ST-Link VCP and is debug-printf only.
...
bool Uart_ReceiveRequest(uint16_t *cycle_num, float *window) {
    ...
    uint8_t b;
    if (HAL_UART_Receive(&huart1, &b, 1, UART_TIMEOUT_MS) != HAL_OK) return false;
    ...
    if (HAL_UART_Receive(&huart1, &rx_buf[2], REQ_PACKET_LEN - 2, UART_TIMEOUT_MS) != HAL_OK) {
        return false;
    }
